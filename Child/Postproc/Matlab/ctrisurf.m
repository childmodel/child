function [tri,meanElev]=ctrisurf(basenm,ts,c)
% CTRISURF: Trisurf from Child triangulation
%  filenm -- name of output file
%  ts -- time slice to plot
%  c -- field to color (0=color by elevation)
%    Usage: [t,z]=ctrisurf(filenm,ts,c), where t=triangles, z=mean elevations
%    Note: if you want to color by something other than elevation, read that
%         data field in (for the same time step) and give it in parameter 'c'. 
%    G. Tucker, 1998
%filesys=['/miami/miami2/gtucker/Mittar/Rhinedata/Papers/Child00/Runs/'];
%filesys=['/miami/miami2/gtucker/Projects/Gullies/'];

a=rand(3)
%filesys=['/nimbus/staff/academic/gtucker/Dev/Child/Examples/']
filesys='';
%filesys=['/niagara/usr/users/gtucker/'];
%filesys=['/arno/usr/users/gtucker/'];
%filesys=['/niagara/data/'];
filenm= [filesys basenm '.nodes' ]
nfid=fopen(filenm,'r');
filenm= [filesys basenm '.tri' ]
tfid=fopen(filenm,'r');
filenm= [filesys basenm '.z' ]
zfid=fopen(filenm,'r');
meanElev=[];
for i=1:ts
  tm = fscanf(nfid,'%f',1)
  tm = fscanf(tfid,'%f',1);
  nn = fscanf(nfid,'%d',1);
  nt = fscanf(tfid,'%d',1);
  t=fscanf(tfid,'%f',[9,nt]); 
  n=fscanf(nfid,'%f',[4,nn]);
  tm = fscanf(zfid,'%f',1);
  nn = fscanf(zfid,'%d',1); 
  z=fscanf(zfid,'%f',[1,nn]);
  meanElev = [meanElev mean(z)]; 
end
tri = [ rot90(t(1,:),3) rot90(t(2,:),3) rot90(t(3,:),3)]+1;
x = n(1,:);
y = n(2,:);
if c==0, c=z; end
if size(z,2)~=nn, error('Color index (c) must have same size as # nodes'),end
trisurf(tri,x,y,z,c)
rot90(meanElev,3);

