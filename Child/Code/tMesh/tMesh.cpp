         ...
         if( cn->Meanders() )
         {
            ...
            for( ce = spokIter.FirstP(); !( spokIter.AtEnd() ); ce = spokIter.NextP() )
            {
               if( ce->getLength() < cn->getHydrWidth()/2.0 )
               {
                  ...
                  tooclose = TRUE;
                  pointtodelete = (tSubNode *) ce->getDestinationPtrNC();
...
CheckTriEdgeIntersect()
{
  ...
  /*cedg = ct->ePtr(i);
            ccedg = getEdgeCompliment( cedg );
            assert( cedg != 0 );
            for( j=0; j<3; j++ )
            {
               spokIter.Reset( ct->pPtr(j)->getSpokeListNC() );
               for( ce = spokIter.FirstP(); !( spokIter.AtEnd() );
                    ce = spokIter.NextP() )
               {
                  if( Intersect( ce, cedg ) )
               ...
               if( crossed )
               {
                  cn = (tSubNode *) ct->pPtr(j);
                  if( cn->Meanders() ) break;
               }
            }
            if( crossed ) break;*/
              /*if( IntersectsAnyEdge( ct->ePtr(i) ) )
                {
                crossed = TRUE;
                break;
                }*/
  /* }
      ...
      if( crossed )
      {
         ...
         if( ct->tPtr(nv) != 0 )
         {
            assert( cedg != 0 );
            ccedg = getEdgeCompliment( cedg );
            subnodePtr = (tSubNode *) cedg->getOriginPtrNC();
            fedg = subnodePtr->GetFlowEdg();
            if( fedg != 0 )
            {
               id0 = fedg->getID();
            }
            else id0 = -1;
            subnodePtr = (tSubNode *) ccedg->getOriginPtrNC();
            fedg = subnodePtr->GetFlowEdg();
            if( fedg != 0 )
            {
               id1 = fedg->getID();
            ...
            if( ( cedg->getID() != id0 || id0 == -1 ) &&
                ( ccedg->getID() != id1 || id1 == -1 ) )
            {*/
                 //basically as a hack: assign node's values to a temp;
  //tempNode = *cn;
                 //delete the node; 
  //           DeleteNode( cn );
                 //add a new node at the original coordinates
  //             xyz = tempNode.getNew3DCoords();
  //           cn = AddNodeAt( xyz );
                 //cn = nodIter.LastActiveP();
  //           assert( cn != 0 );
                 //copy the new node's spokeList to the tempNode;
  //           spokIter.Reset( cn->getSpokeListNC() );
  //           for( ce = spokIter.FirstP(); !(spokIter.AtEnd());
  //                ce = spokIter.NextP() )
  //               tempNode.insertBackSpokeList( ce );
                 //assign value of tempNode to the new node
                 //(behind this simple statement lie jillions of
                 //assignment calls);
  //           *cn = tempNode;
                 ...
                 /*ctop = ct->tPtr(nv);
                   nvopp = ctop->nVOp( ct );
                   assert( nvopp < 3 );
                   newedg->setOriginPtr( ct->pPtr(nv) );
                   newedg->setDestinationPtr( ctop->pPtr(nvopp) );
                   if( !IntersectsAnyEdge( newedg ) &&
                   !newedg->getOriginPtrNC()->
                   EdgToNod( newedg->getDestinationPtrNC() ) )
                   {
                   if( tpIter.Get( ctop->getID() ) )
                   {
                   triptrList.moveToFront( tpIter.NodePtr() );
                   triptrList.removeFromFront( rmtri );
                   }
                   FlipEdge( ct, ctop, nv, nvopp );
                   rmtri = triIter.LastP();
                   triptrList.insertAtBack( rmtri );
  ...
  //else //boundary has been crossed
  /*{
            subnodePtr = (tSubNode *) ct->pPtr(nv);
            subnodePtr->RevertToOldCoords();
         }
      }
   }*/
...
CheckFlowedgCross()
{
   ...
   int flipped;
   int crossed;
   tArray< float > p0, p1, p2;
   tSubNode *pointtodelete, *subnodePtr, *nod, *cn;  
   ...
   do
   {
      ...
      for( ct = triIter.FirstP(); !( triIter.AtEnd() ); ct = triIter.NextP() )
      {
           //cout<<"PA: check triangle "<<ct->id<<", w edges "
           //<<ct->e[0]->id<<", "<<ct->e[1]->id<<", "<<ct->e[2]->id<<endl<<flush;
         cn = (tSubNode *) ct->pPtr(0);
         if( cn->Meanders() ) p0 = cn->getNew2DCoords();
         else p0 = cn->get2DCoords();
         cn = (tSubNode *) ct->pPtr(1);
         if( cn->Meanders() ) p1 = cn->getNew2DCoords();
         else p1 = cn->get2DCoords();
         cn = (tSubNode *) ct->pPtr(2);
         if( cn->Meanders() ) p2 = cn->getNew2DCoords();
         else p2 = cn->get2DCoords();
         if( !PointsCCW( p0, p1, p2 ) )
         {
            crossed = FALSE;
            for( i=0; i<3; i++ )
         ...
         if( crossed )
         {
            ...
            if( ct->tPtr(nv) != 0 )
            {
               ...
               if( cedg->getID() == id0 ||
                   ccedg->getID() == id1 )
               {
                  if( cedg->getID() == id0 ) fedg = cedg;
                  else fedg = ccedg;
                  pointtodelete = (tSubNode *) ct->pPtr(nv);
                  nod = (tSubNode *) fedg->getOriginPtrNC();
                  if( !(pointtodelete->Meanders() &&
                        nod->getDrArea() < pointtodelete->getDrArea()) &&
                      !(pointtodelete->getBoundaryFlag()) )
                  {
                     cout<<"delete point crossed by flowedg"<<endl<<flush;
...
CheckBrokenFlowedg()
{
   ...
   float dis0, dis1;
   tArray< int > npop(3);
   tSubNode *pointtodelete, *subnodePtr, * pedg[2];  
   tEdge * fedg, *tedg[2];
   tTriangle * ct, * trop[3];
   tListIter< tTriangle > triIter( triList );
   ...
   do
   {
           ...
           ct = triPtrIter.NextP() )
      {
         ...
         for( i=0; i<3; i++ )
         {
            if( CheckForFlip( trop[i], npop[i], flip ) )
            {
               pedg[0] = (tSubNode *) ct->pPtr( (i+1)%3 );
               pedg[1] = (tSubNode *) ct->pPtr( (i+2)%3 );
               tedg[0] = ct->ePtr( (i+1)%3 );
               tedg[1] = getEdgeCompliment( tedg[0] );
               fedg = 0;
               ...
               {
                  subnodePtr = (tSubNode *) ct->pPtr(i);
                  dis0 = subnodePtr->DistNew( pedg[0], pedg[1] );
                  subnodePtr = (tSubNode *) trop[i]->pPtr( npop[i] );
                  dis1 = subnodePtr->DistNew( pedg[0], pedg[1] );
                  if( dis0 < dis1 ) pointtodelete = (tSubNode *) ct->pPtr(i);
                  else pointtodelete = (tSubNode *) trop[i]->pPtr( npop[i] );
                  subnodePtr = (tSubNode *) fedg->getOriginPtr();
                  if( pointtodelete->getDrArea() > subnodePtr->getDrArea() )
                      pointtodelete = NULL;
                  else if( pointtodelete->getBoundaryFlag() )
                  {
                     for( i=0; i<2; i++ )
...
MoveNodes()
{
   ...
   tSubNode * cn;  
   tGridListIter< tSubNode > nodIter( nodeList );
   //check for points too close to channel points
   CheckBanksTooClose();
   //check for triangles with edges which intersect (an)other edge(s)
   CheckTriEdgeIntersect();
   //delete node crossed by flowedg
