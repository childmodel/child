...
// ADD MA ************************************
   case kFinneganChannels:
     {
          std::cout << "Finnegan law's chosen for channel width \n";
          kwds = infile.ReadItem( kwds, "HYDR_WID_COEFF_DS" );
          //std::cout << "kwds: " << kwds << std::endl;
          assert( kwds > 0 );
          kdds = infile.ReadItem( kdds, "HYDR_DEP_COEFF_DS" );
...
// ADD MA: next 3 lines *************************************
	       ...
	       width = pow(cn->getChanWidth(), widpow) * kwdspow * pow(qpsec, ewstn);
               ...
               //std::cout << "Channel width is:" << width << "\n";
...
// ADD MA *****************************************
      if( miChannelType==kFinneganChannels  && cn->calcSlope()!=0 )
      {
         ...
         // std::cout << "Bankful width =" << width << "\n";
      }
      else
          width = kwds * pow(qbf, ewds);
// END ADD MA *****************************************
      // width = kwds * pow(qbf, ewds);  This line has been replaced by what precedes (MA)
